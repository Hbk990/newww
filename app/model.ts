...
export function matches(value:string,query:string){const hay=normalize(value);return normalize(query).trim().split(/\s+/).every(q=>hay.includes(q)|| (q.length>=3&&hay.split(/[^a-z0-9]+/).some(w=>distance(w,q)<=(q.length>5?2:1))))}
