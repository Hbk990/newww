...
final class CheckoutController
{
    public function form(Request $request): void
    {
        ...
        View::render('storefront/checkout',['title'=>'Checkout','store'=>$store,'checkoutToken'=>(new CheckoutTokenService)->issue((int)$store['id']),'meta'=>['robots'=>'noindex,nofollow','canonical'=>store_absolute_url($store,'checkout')]],'storefront');
    }

    public function create(Request $request): void
    {
        $store = $this->activeStore((string)$request->route('storeSlug'));
        if ((new RateLimiter)->tooMany('checkout',(int)$store['id'].'|'.$request->ip(),10,3600)) { Session::flash('error','Too many checkout attempts. Please wait and try again.'); Response::redirect(store_url($store,'checkout'),303); }
        ...
        if($errors)$this->fail($store,implode(' ',$errors),$request);
        try{$order=(new OrderService)->create($store,$customer,$lines,$token,$discountCode!==''?$discountCode:null);}catch(\DomainException$e){$this->fail($store,$e->getMessage(),$request);}
        Response::redirect(store_url($store,'order/'.$order['reference'].'/continue').'?token='.rawurlencode($token),303);
    }
    ...
    public function reorder(Request$request):void
    {
        ...
        $repo=new OrderRepository;$order=$repo->publicOrder((int)$store['id'],$reference,hash('sha256',$token));if(!$order)Response::abort(404);$reorder=$repo->reorder((int)$store['id'],(int)$order['id']);
        foreach($reorder['items']as&$item){$item['url']=store_url($store,'product/'.$item['slug']);unset($item['slug']);}unset($item);
    ...
    private function activeStore(string$slug,string$redirectSuffix=''):array{$repo=new StorefrontRepository;$store=$repo->storeBySlug($slug);if(!$store){$target=$repo->redirectTarget($slug);if($target&&$redirectSuffix!=='')Response::redirect(store_absolute_url(['slug'=>$target],$redirectSuffix),301);Response::abort(404);}if($store['status']!=='ACTIVE')Response::abort(404);return$store;}
    private function fail(array$store,string$message,Request$request):never{Session::put('_old',array_diff_key($request->all(),['_token'=>true,'cart'=>true,'idempotency_token'=>true]));Session::flash('error',$message);Response::redirect(store_url($store,'checkout'),303);}
}
