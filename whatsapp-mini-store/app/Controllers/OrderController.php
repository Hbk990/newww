...
final class OrderController
{
    ...
    public const CANCEL_REASONS=['HIGH_DELIVERY_FEE'=>'Delivery fee was too high','CUSTOMER_UNREACHABLE'=>'Could not reach the customer','CHANGED_MIND'=>'Customer changed their mind','OUT_OF_STOCK'=>'Item turned out to be unavailable','DUPLICATE_ORDER'=>'Duplicate order','OTHER'=>'Other'];
    ...
    public function show(Request$request):void{$store=(new TenantContext)->store();$order=(new OrderRepository)->merchantOrder((int)$store['id'],(string)$request->route('reference'));if(!$order)Response::abort(404);View::render('merchant/orders/show',['title'=>$order['reference'],'store'=>$store,'order'=>$order,'nextStatuses'=>$this->next($order['status']),'cancelReasons'=>self::CANCEL_REASONS],'merchant');}
    public function status(Request$request):void{$store=(new TenantContext)->store();$reference=(string)$request->route('reference');$next=strtoupper((string)$request->input('status'));if(!in_array($next,self::STATUSES,true))Response::abort(400);$reasonCode=null;$reasonNote=null;if($next==='CANCELLED'){$reasonCode=strtoupper((string)$request->input('reason_code'));if(!isset(self::CANCEL_REASONS[$reasonCode])){Session::flash('error','Choose a cancellation reason.');Response::redirect('/merchant/orders/'.$reference,303);}$reasonNote=mb_substr(trim((string)$request->input('reason_note')),0,300);$reasonNote=$reasonNote===''?null:$reasonNote;}$repo=new OrderRepository;try{if(!$repo->updateStatus((int)$store['id'],$reference,$next,(int)Auth::id(),$reasonCode,$reasonNote))Response::abort(404);}catch(\DomainException$e){Session::flash('error',$e->getMessage());Response::redirect('/merchant/orders/'.$reference,303);}$order=$repo->merchantOrder((int)$store['id'],$reference);(new AuditLogRepository)->record((int)Auth::id(),'order.status_changed','order',(int)$order['id'],['store_id'=>$store['id'],'reference'=>$reference,'status'=>$next,'reason_code'=>$reasonCode]);Session::flash('success','Order status updated to '.ucfirst(strtolower($next)).'.');Response::redirect('/merchant/orders/'.$reference,303);}
