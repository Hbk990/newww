...
{
    public function create(array $store, array $customer, array $rawLines, string $idempotencyToken): array
    {
        ...
        try {
            ...
            foreach ($lines as $line) {
                ...
                $items[] = ['product_id'=>(int)$product['id'],'variant_id'=>$variant?(int)$variant['id']:null,'product_name'=>$product['name'],'product_slug'=>$product['slug'],'sku_snapshot'=>$variant['sku']??$product['sku'],'variant_label'=>$variant['label']??null,'unit_price'=>$unit,'quantity'=>$line['quantity'],'line_total'=>Money::fromMinor($lineMinor)];
            }
            $reference = $this->reference($pdo); $total = Money::fromMinor($totalMinor);$customerId=(new CustomerRepository)->upsert($pdo,$storeId,$customer['name'],$customer['phone']);
            $insert = $pdo->prepare("INSERT INTO orders (store_id,customer_id,reference,idempotency_key_hash,customer_name,customer_phone,delivery_address,notes,currency_code,subtotal,total,status,placed_at,status_updated_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,'NEW',UTC_TIMESTAMP(),UTC_TIMESTAMP(),UTC_TIMESTAMP(),UTC_TIMESTAMP())");
            $insert->execute([$storeId,$customerId,$reference,$hash,$customer['name'],$customer['phone'],$customer['address'],$customer['notes'],$store['currency_code'],$total,$total]); $orderId = (int)$pdo->lastInsertId();
