...
final class ProductRepository
{
    ...
    /** Variants with finite, tracked stock at or below the store's alert threshold (0 first, i.e. out of stock). */
    public function lowStock(int $storeId, int $threshold): array
    {
        $s = Database::connection()->prepare("SELECT pv.id,pv.label,pv.stock_quantity,p.id product_id,p.name product_name FROM product_variants pv JOIN products p ON p.id=pv.product_id AND p.store_id=pv.store_id WHERE pv.store_id=? AND pv.stock_quantity IS NOT NULL AND pv.stock_quantity<=? AND p.deleted_at IS NULL AND p.status<>'ARCHIVED' ORDER BY pv.stock_quantity ASC,p.name LIMIT 50");
        $s->execute([$storeId, $threshold]);
        return $s->fetchAll();
    }
