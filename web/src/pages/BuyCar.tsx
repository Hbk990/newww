...
export default function BuyCar() {
  ...
  return (
    <>
      <PageHeader title="Buy a car" sub="Everything here is in USD — it converts to local currency when the shipment arrives" />
      ...
      <div className="grid cols-2">
        <div>
          <Card title="Supplier">
            <Field label="Who did you buy it from?">
            ...
            {supplier?.country === 'USA' && (
              <Alert kind="info">USA supplier — no tax applies to this purchase.</Alert>
            )}
            {supplier?.country === 'CANADA' && !taxApplies && (
              <Alert kind="info">
                This Canadian supplier invoices the car price only, so there is no tax to enter.
              </Alert>
            )}
          ...
          <Card title="The car">
            ...
            <Field label="Model" help={models.length > 0 ? `${models.length} models known for ${makeName}. You can also type one that is not listed.` : 'Type the model.'}>
              <input
                list="models"
                value={modelName}
                placeholder="CLA 300"
            ...
            <div className="row">
              <Field label="Year">
                <input type="number" value={year} onChange={(e) => setYear(e.target.value)} min={1950} max={new Date().getFullYear() + 1} />
              </Field>
              <Field label="Colour">
        ...
        <div>
          <Card title="Money">
            ...
            {expenses.map((expense, index) => (
              <div className="row" key={index}>
                ...
                <div className="actions">
                  <button className="secondary small" onClick={() => setExpenses(expenses.filter((_, i) => i !== index))}>
                    Remove
                  </button>
                </div>
              </div>
            ...
            <button className="secondary small" onClick={() => setExpenses([...expenses, { amountUsd: '', note: '' }])}>
              + Add an expense
            </button>
          </Card>
          ...
          <button onClick={() => void run()} disabled={busy || !ready} style={{ width: '100%' }}>
