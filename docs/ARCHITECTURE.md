...
`available = on_hand - reserved`. Adding to cart creates a row in `inventory_reservations` with an `expires_at`; a sweeper releases stale ones. Every decrement runs inside a transaction that takes `SELECT ... FOR UPDATE` on the inventory row, so two buyers racing for the last unit produce one sale and one honest "out of stock" — not oversell. All movements append to `inventory_ledger`, so stock is auditable rather than just a mutable number.
